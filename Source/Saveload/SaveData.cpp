...
FUSaveData::FUSaveData(ASaveTestActor* Actor)
{
	this->ActorLabel = Actor->GetActorLabel();
	this->Location = Actor->GetActorLocation();
	this->var = Actor->var;
...
FString FUSaveData::JsonSerialize()
{
	FString str = TEXT("");
	FJsonObjectConverter::UStructToJsonObjectString(FUSaveData::StaticStruct(), this, str, 0, 0);
	//UE_LOG(LogTemp, Log, TEXT("Serialized : %s"), *str);
	return str;
}

//Deserialize the object passed by argument and set the datas to actor
void FUSaveData::SetDeserializedData(const TSharedPtr<FJsonObject>& JsonObject, ASaveTestActor* Actor)
{
...
void FUSaveData::JsonDeserialize(const TSharedPtr<FJsonObject>& JsonObject)
{
...
void FUSaveData::JsonDeserializeLocation(const TSharedPtr<FJsonObject>& JsonObject)
{
	TArray<FString> Keys = TArray<FString>();
	for (auto& Field : JsonObject->Values)
	{
